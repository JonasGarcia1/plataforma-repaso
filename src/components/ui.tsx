...
export function LessonRow({lesson,index}:{lesson:Lesson;index?:number}) {const {progress}=useProgress();return <div className="lesson-row"><span className="lesson-number">{index!==undefined?String(index+1).padStart(2,'0'):<BookOpen size={17}/>}</span><Link to={`/leccion/${lesson.id}`} className="lesson-row-main"><strong>{lesson.title}</strong><span>{lesson.description}</span></Link><div className="lesson-row-meta"><span className="time"><Clock3 size={13}/>{lesson.minutes} min</span><StatusBadge status={progress.statuses[lesson.id]}/></div><ChevronRight size={17} className="muted"/></div>;}
